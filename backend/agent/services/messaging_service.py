...
class MessagingService:
    ...
    def __init__(self, agent, telegram_service, database_service):
        """
    ...
    async def process_response(
        ...
        try:
            ...
            context = self._build_agent_context(message_data, user.id)
            
            print(f"[MESSAGING_SERVICE] Context prepared:")
            print(f"  - User DB ID: {user.id}")
            print(f"  - Has photo: {context['has_photo']}")
    ...
    def _build_agent_context(
        ...
        """
        Construye el contexto necesario para el agente de IA.
        
        Args:
            message_data: Datos extraídos del mensaje de Telegram
            user_db_id: ID del usuario en la base de datos
            
        Returns:
            Dict con el contexto completo para el agente
        """
        return {
            "telegram_id": message_data["telegram_id"],
            "username": message_data["username"],
